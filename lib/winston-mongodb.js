 ...
 * a string.
 * @param {number=10} options.reconnectTries Will retry to reconnect to the
 * database in case of fail during initialization. Works only if `db` is
 * a string.
  ...
  function connectToDatabase(logger) {
    return mongodb.MongoClient.connect(logger.db, logger.options
      ...
      if (this.reconnectTries <= 0) {
        console.error('winston-mongodb: error initialising logger, max count retries reached', err);
        self.mongoClient.error();
      } else {
...
/**
 * Error when it fails to connect to mongoDB server.
 * Used by winston Logger.error on transports.
 */
MongoDB.prototype.error = function () {
  this.logDb = null;
  this.reconnectTries = options.reconnectTries;
  if (!this.mongoClient || this.leaveConnectionOpen) {
    return;
